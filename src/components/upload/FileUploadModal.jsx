    ...
    <Modal open={open} onClose={() => setOpen(false)}>
      <ModalPopup>
        ...
        <form onSubmit={handleUpload}>
          <ModalHeading>
            <h3>
              {uploading ? "Uploading..." : "Select the file to be uploaded"}
            </h3>
          </ModalHeading>
          <ModalBody>
            {uploading ? (
              ...
              <>
                <div className="modal__file" style={{border:"1px dashed #F2613F"}}>
                  <p>{selectedFile ? selectedFile : ""}</p>
                  <label htmlFor="file">
                    <UploadFileIcon /> Select a file 
                  </label>
                  <input id="file" type="file" onChange={handleFile}  />
                </div>
                <input type="submit" className="modal__submit" value={"Upload"} style={{backgroundColor:"#F2613F"}} />
              </>
            )}
          </ModalBody>
        </form>
